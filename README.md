...
PrettyWorks의 API 서버입니다. 인증 · 도메인 데이터 · AI Agent 게이트웨이를 담당합니다.

프로젝트 전체 소개와 시스템 아키텍처는 👉 **[조직 프로필](https://github.com/Pretty-Works)**
...
├── tool/             FastAPI가 호출하는 내부 도구 API (조회 13 · 쓰기 9)
├── suggestion/       제안 생성
├── summary/          프로젝트 요약
├── meetingdraft/     회의록 초안
└── shared/           첨부 · 한도 · 내부 인증
...
각 모듈은 `api / application / domain / persistence`로 나뉘고, 외부 시스템을 호출하는 모듈에는 `gateway`가 추가됩니다. 다른 도메인이 `controller / service / repository` 구조인 것과 다른데, **Agent는 외부 시스템 연동이 본질이라 경계를 명시적으로 분리**했습니다.
...
35개 테스트 중 32개가 `agent` 패키지에 있습니다. 복잡도가 몰린 곳에 검증을 집중했습니다.
...
위 **Agent 연동** 섹션에서 설명한 판정 규칙과 한도는 대부분 여기서 검증됩니다.
...
이 저장소가 일반적인 Spring API 서버와 다른 지점입니다.
...
핵심은 `X-Run-Id`입니다. **Agent는 자신이 어떤 실행 중인지만 알고, 그 실행의 주인이 누구인지는 서버가 결정합니다.** 요청 본문에 임의의 `user_id`를 넣어도 사용되지 않습니다.
...
구현 중 내린 판단과 근거를 문서로 남겼습니다.

**Agent 연동**
[실행을 별도 스레드 풀로 격리한 이유](https://app.notion.com/p/3bf574a060ac802faf2be238eb43506a) ·
[승인 게이트와 Approval Token](https://app.notion.com/p/3bf574a060ac803d937ff7ccb65de35f) ·
[Run 상태 관리와 SSE 재연결](https://app.notion.com/p/3bf574a060ac806ebb77cc63da829313)

**데이터 관리**
[도메인 간 참조를 ID로 고정](https://app.notion.com/p/3bf574a060ac80739d37faea3c8a469d) ·
[알림 저장의 트랜잭션 경계](https://app.notion.com/p/3bf574a060ac80b98a19d785b88a9f64) ·
[중복 생성과 동시 수정의 제어 방식 분리](https://app.notion.com/p/3bf574a060ac8033ad78c29ec92101da)

**인증 · 세션**
[JWT 세션 무효화 — 차단 단위](https://app.notion.com/p/3bf574a060ac80cb9f39c21694e48601) ·
[로그아웃 — 서버 세션과 클라이언트 캐시](https://app.notion.com/p/3bf574a060ac80e9903fe3faf825acc1)

**도메인 규칙**
[권한 규칙의 Policy 분리](https://app.notion.com/p/3bf574a060ac8090b1cbeaa4174d1070) ·
[마일스톤 순서 불변식](https://app.notion.com/p/3bf574a060ac803d9e34c07178ef5b96) ·
[할 일 권한](https://app.notion.com/p/3bf574a060ac806aa867cafa805449ff)
