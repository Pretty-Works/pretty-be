...
@Tag(name = "프로젝트 AI 요약", description = """
        ...
        ② 조회했는데 저장된 게 없거나, 그사이 프로젝트 데이터가 바뀌었으면 그 자리에서 다시 만듭니다.
        ③ 갱신 API를 부르면 만듭니다.

        [바뀐 걸 어떻게 아나]
        할 일·마일스톤·지출·게시글·회의록·프로젝트 정보의 변경을 조회 시점에 감지합니다.
        게시글을 쓰거나 할 일을 체크하면 다음 조회에서 배너가 따라옵니다 — 프론트가 알릴 필요 없습니다.
        다만 마지막 생성으로부터 최소 간격(기본 10분)이 지나야 다시 만듭니다. LLM 호출이 곧 비용이라,
        활발한 프로젝트에서 진입할 때마다 새로 만들지 않기 위한 제동입니다.
        """)
    ...
    @Operation(
            ...
            description = """
                    **프론트가 부르는 것은 이것 하나입니다. projectId만 넘기면 됩니다.**
                    ...
                    다만 **저장된 요약이 없거나 프로젝트 데이터가 바뀌었으면 그 자리에서 다시 만들기 때문에
                    수 초가 걸립니다.** 그래서 배너 자리에는 스켈레톤을 띄워 두세요.

                    [데이터를 바꾼 뒤]
                    게시글 등록·할 일 완료·지출 등록 뒤에 조회를 다시 부르면 배너도 따라 바뀝니다.
                    변경 감지는 서버가 하므로 프론트가 무엇이 바뀌었는지 알려 줄 필요는 없습니다.
                    단 마지막 생성으로부터 최소 간격(기본 10분) 안이면 이전 배너가 그대로 나옵니다.
