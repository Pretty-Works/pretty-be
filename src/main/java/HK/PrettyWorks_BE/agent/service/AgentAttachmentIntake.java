...
public class AgentAttachmentIntake {
     ...
     *
     * <p>txt 외 형식을 열려면 설정에 확장자를 추가하면 됩니다. 표에 없는 확장자를 설정에 쓰면
     * 기동이 실패하므로, 새 형식은 반드시 이 표에 먼저 등록해야 합니다.</p>
     */
    private static final Map<String, FileType> KNOWN_TYPES = Map.of(
            "txt", new FileType("text/plain", AgentFileEncoding.TEXT),
            "md", new FileType("text/markdown", AgentFileEncoding.TEXT),
            "csv", new FileType("text/csv", AgentFileEncoding.TEXT),
            "json", new FileType("application/json", AgentFileEncoding.TEXT),
