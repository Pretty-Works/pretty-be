...
// 일정 API의 Swagger 문서 전용 인터페이스. 컨트롤러(ScheduleController)가 implements 하며,
...
public interface ScheduleApi {
    ...
    ResponseEntity<ScheduleCreateResponse> create(
            @Parameter(hidden = true) Long writerId,
    ...
    ResponseEntity<ScheduleListResponse> list(
            @Parameter(hidden = true) Long userId,
            @Parameter(description = "조회 시작일 (yyyy-MM-dd)", example = "2026-08-01") LocalDate from,
            @Parameter(description = "조회 종료일 (yyyy-MM-dd)", example = "2026-08-31") LocalDate to,
            @Parameter(description = "함께 조회할 사용자 id 목록(선택). 본인은 항상 포함된다.") List<Long> userIds
    );
    ...
    Void delete(
            @Parameter(hidden = true) Long userId,
            @Parameter(description = "삭제할 일정 id") Long scheduleId
    );
    ...
    Void leave(
            @Parameter(hidden = true) Long userId,
            @Parameter(description = "나갈 일정 id") Long scheduleId
    );
    ...
    ResponseEntity<ScheduleUpdateResponse> update(
            @Parameter(hidden = true) Long userId,
            @Parameter(description = "수정할 일정 id") Long scheduleId,
